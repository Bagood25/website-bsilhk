...
@section('title', 'Berita KLHK')

@section('content')
    <div class="bg-gray-900 py-16 px-4 text-center relative">
        <div class="max-w-7xl mx-auto z-10 relative">
            <h1 class="text-4xl font-extrabold text-white sm:text-5xl">Berita KLHK</h1>
            <p class="mt-2 text-sm text-gray-300">Beranda &gt; Berita &gt; Berita KLHK</p>
        </div>
    </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                @forelse ($news as $item)
                    <div class="bg-white rounded-lg shadow-lg overflow-hidden flex flex-col transition-transform transform hover:-translate-y-2">
                        <img src="{{ $item->gambar ? asset('storage/' . $item->gambar) : 'https://placehold.co/600x400/A7F3D0/1E3A8A?text=Berita+KLHK' }}" alt="Gambar Berita: {{ $item->judul }}" class="w-full h-48 object-cover">
