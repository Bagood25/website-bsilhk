            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                @forelse ($news as $item)
                    <div class="bg-white rounded-lg shadow-lg overflow-hidden flex flex-col transition-transform transform hover:-translate-y-2">
                        <img src="{{ $item->gambar ? asset('storage/' . $item->gambar) : 'https://placehold.co/600x400/A7F3D0/1E3A8A?text=Berita+BSI' }}" alt="Gambar Berita: {{ $item->judul }}" class="w-full h-48 object-cover">
